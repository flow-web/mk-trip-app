-- ════════════════════════════════════════════════════════════════════════════
-- MK Trip — setup complet à exécuter une seule fois dans Supabase SQL Editor
-- (https://supabase.com/dashboard/project/bprcrsqeaqudbwumztoy/sql/new)
--
-- ⚠  CECI WIPE TOUT LE SCHEMA `public` (legacy Expo inclus).
-- Combine : drop + foundation (20260520000000_initial.sql)
--                + hero (20260520_refonte_hero.sql)
--                + trigger profil auto + backfill profiles existants.
-- ════════════════════════════════════════════════════════════════════════════
...
-- Auto-création du profile au signup
...
create table public.trips (
  ...
  hero_image_url text,
  hero_image_uploaded boolean default false,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
begin
  ...
  if v_owner is null then raise exception 'TRIP_NOT_FOUND'; end if;
...
create policy "trips_update_editor" on public.trips for update using (
  exists (select 1 from public.trip_members where trip_id = id and user_id = auth.uid() and role in ('owner','editor'))
);
create policy "trips_delete_owner" on public.trips for delete using (
  exists (select 1 from public.trip_members where trip_id = id and user_id = auth.uid() and role = 'owner')
);
...
-- ─── Bucket Storage trip-covers + RLS ─────────────────────────────────────────
insert into storage.buckets (id, name, public)
values ('trip-covers', 'trip-covers', true)
on conflict (id) do nothing;

drop policy if exists "trip_covers_member_write" on storage.objects;
drop policy if exists "trip_covers_member_update" on storage.objects;
drop policy if exists "trip_covers_member_delete" on storage.objects;
...
-- ─── Realtime : autoriser les broadcast sur les tables clés ──────────────────
alter publication supabase_realtime add table public.trips, public.trip_members,
  public.days, public.activities, public.activity_completions, public.spots,
  public.expenses, public.expense_splits, public.checklist_items,
  public.checklist_completions, public.guide_cards;

-- ─── Backfill profiles pour les users déjà inscrits ──────────────────────────
insert into public.profiles (id, display_name)
